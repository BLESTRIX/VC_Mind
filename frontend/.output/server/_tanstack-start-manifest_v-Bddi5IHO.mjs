//#region node_modules/.nitro/vite/services/ssr/assets/_tanstack-start-manifest_v-Bddi5IHO.js
var tsrStartManifest = () => ({ routes: {
	__root__: {
		filePath: "E:/VC Mind/VC_Mind/frontend/src/routes/__root.tsx",
		children: [
			"/",
			"/applications/$id",
			"/applications/new",
			"/diligence/$token"
		],
		preloads: [
			"/assets/index-BN0LwNDV.js",
			"/assets/button-CJW0GcWD.js",
			"/assets/lazyRouteComponent-C58aFwGe.js",
			"/assets/AuthProvider-ClSKVbaD.js",
			"/assets/applications._id-DzhXN9mg.js",
			"/assets/diligence._token-CVtJR_Hv.js"
		],
		scripts: [{ attrs: {
			type: "module",
			async: !0,
			src: "/assets/index-BN0LwNDV.js"
		} }]
	},
	"/": {
		filePath: "E:/VC Mind/VC_Mind/frontend/src/routes/index.tsx",
		children: void 0,
		preloads: [
			"/assets/routes-DIleWUDJ.js",
			"/assets/dist-9OWGo0s-.js",
			"/assets/primitives-ZLVhamXl.js",
			"/assets/select-NIjf7x1W.js",
			"/assets/table-DnHTdKua.js"
		]
	},
	"/applications/$id": {
		filePath: "E:/VC Mind/VC_Mind/frontend/src/routes/applications.$id.tsx",
		children: void 0,
		preloads: [
			"/assets/applications._id-CXiF7fOe.js",
			"/assets/useNavigate-Bn5NA8Gp.js",
			"/assets/dist-9OWGo0s-.js",
			"/assets/primitives-ZLVhamXl.js",
			"/assets/applications-zKAvIiRj.js",
			"/assets/table-DnHTdKua.js",
			"/assets/textarea-BkQnMFGX.js"
		]
	},
	"/applications/new": {
		filePath: "E:/VC Mind/VC_Mind/frontend/src/routes/applications.new.tsx",
		children: void 0,
		preloads: [
			"/assets/applications.new-CQE50z0k.js",
			"/assets/useNavigate-Bn5NA8Gp.js",
			"/assets/dist-9OWGo0s-.js",
			"/assets/primitives-ZLVhamXl.js",
			"/assets/applications-zKAvIiRj.js",
			"/assets/select-NIjf7x1W.js",
			"/assets/textarea-BkQnMFGX.js"
		]
	},
	"/diligence/$token": {
		filePath: "E:/VC Mind/VC_Mind/frontend/src/routes/diligence.$token.tsx",
		children: void 0,
		preloads: [
			"/assets/diligence._token-BYuDGanG.js",
			"/assets/primitives-ZLVhamXl.js",
			"/assets/textarea-BkQnMFGX.js"
		]
	}
} });
//#endregion
export { tsrStartManifest };
