...
export class GroqProvider implements AIProvider {
  async generateStructured<T>(input: GenerateStructuredInput<T>): Promise<AIResult<T>> {
    const env = getEnv(); const started = Date.now(); const schema = z.toJSONSchema(input.schema);
    const body = { model: input.model, messages: [{ role: 'system', content: `${input.systemPrompt}\n\nReturn exactly one JSON object matching this JSON Schema. Do not include markdown fences or commentary.\n${JSON.stringify(schema)}` }, { role: 'user', content: input.userPrompt }], temperature: input.temperature ?? .1, response_format: { type: 'json_object' }, stream: false };
    ...
    if (!response.ok) throw new AppError('AI_PROVIDER_ERROR', payload.error?.message ?? `Groq returned ${response.status}`, response.status >= 500 ? 503 : 502, { status: response.status, model: input.model }, response.status === 429 || response.status >= 500);
