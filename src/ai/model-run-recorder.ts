...
export class ModelRunRecorder {
  ...
  async generate<T>(input: { applicationId?: string; claimId?: string; runType: string; model: string; prompt: PromptDefinition; userPrompt: string; schema: z.ZodType<T>; schemaName: string; metadata?: Record<string, unknown> }): Promise<T> {
    ...
    for (const model of models) {
      ...
      try {
        const result = await this.provider.generateStructured({ model, systemPrompt: input.prompt.systemPrompt, userPrompt: input.userPrompt, schema: input.schema, schemaName: input.schemaName, temperature: .1, ...(input.metadata ? { metadata: input.metadata } : {}) });
        ...
        if (model === getEnv().AI_MODEL_STRONG || (error instanceof AppError && !error.retryable)) break;
      }
    }
