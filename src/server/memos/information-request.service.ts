...
import { PRIVATE_DOCUMENT_TYPES } from '../private-diligence/private-diligence.types.js';
...
export class InformationRequestService {
  ...
  async generate(applicationId: string, requestedBy?: string) {
    ...
    const rows = output.requests.map((request) => ({ application_id: applicationId, claim_id: request.relatedClaimId ?? null, requested_by: requestedBy ?? null, title: request.title, description: `[${request.priority}] ${request.description}`, requested_document_type: request.requestedDocumentType ? (PRIVATE_DOCUMENT_TYPES.includes(request.requestedDocumentType as typeof PRIVATE_DOCUMENT_TYPES[number]) ? request.requestedDocumentType : 'other') : null, status: 'requested' as const }));
