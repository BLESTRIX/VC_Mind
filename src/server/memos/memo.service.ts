...
export class MemoService {
  ...
  async generate(applicationId: string, revision?: { previous: MemoOutput; skeptic: unknown; previousMemoId: string }) {
    ...
    const structured = { application, claims: claims ?? [], evidence: evidence ?? [], scores: scores ?? [], deal, ...(revision ? { draftMemo: revision.previous, skepticReview: revision.skeptic } : {}) };
    const prompt = revision ? prompts.memoRevision : prompts.memoGeneration;
    let output: MemoOutput | undefined;
    let lastError: unknown;
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        output = await this.ai.generate({ applicationId, runType: revision ? 'memo_revision' : 'memo_generation', model: getEnv().AI_MODEL_STRONG, prompt, userPrompt: JSON.stringify(structured).slice(0, getEnv().MAX_MEMO_INPUT_CHARS), schema: memoSchema, schemaName: revision ? 'memo_revision' : 'investment_memo' });
