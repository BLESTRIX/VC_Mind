...
export class SkepticService {
  ...
  async review(applicationId: string, memoId: string, memoOutput: unknown) {
    ...
    const output = await this.ai.generate({ applicationId, runType: 'skeptic_review', model: getEnv().AI_MODEL_STRONG, prompt: prompts.skepticReview, userPrompt: JSON.stringify({ memo: memoOutput, claims, scores, evidence }).slice(0, getEnv().MAX_MEMO_INPUT_CHARS), schema: skepticSchema, schemaName: 'skeptic_review' });
