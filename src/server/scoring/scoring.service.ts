...
export function hasCriticalContradiction(claims: Array<{ importance: string; verification_status: string }>): boolean { return claims.some((claim) => claim.importance === 'high' && claim.verification_status === 'contradicted'); }

export class ScoringService {
  async calculate(applicationId: string) {
    const db = getServiceClient();
    const [{ data: jobs }, { data: claims }, { data: deal }, { data: thesis }, { data: coverage }] = await Promise.all([
      db.from('processing_jobs').select('job_type,result').eq('application_id', applicationId).eq('status', 'completed'), db.from('claims').select('importance,verification_status,category,checkable').eq('application_id', applicationId), db.from('deal_economics').select('*').eq('application_id', applicationId).maybeSingle(), db.from('scores').select('*').eq('application_id', applicationId).eq('dimension', 'thesis_fit').eq('is_current', true).maybeSingle(), db.rpc('application_evidence_coverage', { p_application_id: applicationId })
    ]);
    ...
    const { error } = await db.from('scores').insert(rows); if (error) throw new AppError('INTERNAL_ERROR', 'Scores could not be stored', 500, { database: error.message });
