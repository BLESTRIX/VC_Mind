...
export type ReconciliationResult = 'matched' | 'approximately_matched' | 'mismatched' | 'insufficient_data' | 'not_applicable';
export type ReconciliationSeverity = 'low' | 'medium' | 'high' | 'critical';
export type ClosingCheckStatus = 'not_started' | 'in_progress' | 'completed' | 'waived' | 'blocked';
export type FactorAssessmentLevel = 'none'|'weak'|'moderate'|'strong'|'exceptional';
export type EvidenceClusterReason = 'exact_content_match'|'near_duplicate_content'|'same_press_release'|'same_original_publisher'|'same_corporate_owner'|'founder_controlled'|'manual_grouping'|'unique_source';
...
export type DocumentRow = Audit & { id: string; application_id: string; uploaded_by: string | null; information_request_id: string | null; document_type: string; storage_bucket: string; storage_path: string; original_filename: string | null; mime_type: string | null; file_size_bytes: number | null; sha256_hash: string | null; version: number; is_current: boolean; processing_status: string; page_count: number | null; extracted_text: Json | null; document_date: string | null; source_system: string | null; authenticity_status: string; manual_review_status: string; uploaded_at: string; processed_at: string | null };
...
export type EvidenceSourceRow = Audit & { id: string; canonical_url: string; source_title: string | null; source_domain: string | null; source_type: EvidenceSourceType; publisher_name: string | null; published_at: string | null; retrieved_at: string; content_hash: string | null; snapshot_text: string | null; snapshot_storage_path: string | null; founder_controlled: boolean; authoritative_source: boolean; independence_cluster: string | null;cluster_reason:EvidenceClusterReason|null;cluster_similarity:number|null;canonical_source_id:string|null;original_publisher:string|null;counts_as_independent:boolean };
export type EvidenceRow = Audit & { id: string; claim_id: string; evidence_source_id: string; search_query_id: string | null; document_id: string | null; document_extraction_id: string | null; relationship: EvidenceRelationship; excerpt: string; excerpt_hash: string | null; observed_value: Json | null; period_start: string | null; period_end: string | null; source_reference: string | null; source_quality: number | null; entity_match: number | null; freshness: number | null; evidence_completeness: number | null; model_confidence: number | null; authenticity_status: string; validation_status: 'pending' | 'valid' | 'invalid'; validation_error: string | null; human_review_status: string | null; human_reviewed_by: string | null; human_reviewed_at: string | null };
...
export type ScoreFactorRow={id:string;application_id:string;score_id:string|null;dimension:Exclude<ScoreDimension,'overall'>;factor_key:string;factor_label:string;assessment_level:FactorAssessmentLevel;score:number;maximum_score:number;explanation:string;supporting_claim_ids:string[];supporting_evidence_ids:string[];missing_data:boolean;rubric_version:string;model_run_id:string|null;is_current:boolean;created_at:string};
...
export type ApplicationStageEventRow = { id: string; application_id: string; stage: ApplicationStage; attempt_number: number; status: JobStatus; started_at: string | null; completed_at: string | null; duration_ms: number | null;budget_seconds:number|null;fallback_actions:Json; error_message: string | null; triggered_by: string | null; metadata: Json | null; created_at: string };
...
export type DiligenceAccessTokenRow = { id: string; application_id: string; token_hash: string; expires_at: string; revoked_at: string | null; last_accessed_at: string | null; created_by: string | null; created_at: string };
export type DocumentExtractionRow = { id: string; document_id: string; extractor_type: string; extractor_version: string; status: 'pending'|'processing'|'completed'|'failed'; structured_data: Json; warnings: Json; confidence: number | null; model_run_id: string | null; is_current: boolean; created_at: string };
export type ClaimReconciliationRow = { id: string; application_id: string; claim_id: string; claimed_value: Json | null; observed_value: Json | null; unit: string | null; period_start: string | null; period_end: string | null; result: ReconciliationResult; variance_percentage: number | null; material: boolean; severity: ReconciliationSeverity; explanation: string; supporting_document_ids: string[]; reconciliation_version: string; is_current: boolean; created_at: string };
export type ClosingChecklistItemRow = Audit & { id: string; application_id: string; check_type: string; status: ClosingCheckStatus; blocking: boolean; evidence_document_id: string | null; reviewed_by: string | null; reviewed_at: string | null; notes: string | null };
export type DiligenceResponseRow = { id: string; application_id: string; information_request_id: string; response_text: string; created_at: string };
export type DiligenceAuditEventRow = { id: string; application_id: string; actor_type: 'founder_token'|'staff'|'worker'; actor_id: string | null; action: string; target_type: string | null; target_id: string | null; metadata: Json; created_at: string };
...
export interface Database {
  public: {
    Tables: {
      ...
      score_factors:Table<ScoreFactorRow,'application_id'|'dimension'|'factor_key'|'factor_label'|'assessment_level'|'score'|'maximum_score'|'explanation'|'rubric_version'>;
      ...
      diligence_access_tokens: Table<DiligenceAccessTokenRow, 'application_id'|'token_hash'|'expires_at'>;
      document_extractions: Table<DocumentExtractionRow, 'document_id'|'extractor_type'|'extractor_version'|'status'>;
      claim_reconciliations: Table<ClaimReconciliationRow, 'application_id'|'claim_id'|'result'|'severity'|'explanation'|'reconciliation_version'>;
      closing_checklist_items: Table<ClosingChecklistItemRow, 'application_id'|'check_type'>;
      diligence_responses: Table<DiligenceResponseRow, 'application_id'|'information_request_id'|'response_text'>;
      diligence_audit_events: Table<DiligenceAuditEventRow, 'application_id'|'actor_type'|'action'>;
    };
    Views: {
    ...
    Enums: {
      ...
      reconciliation_result: ReconciliationResult; reconciliation_severity: ReconciliationSeverity; closing_check_status: ClosingCheckStatus;
    };
